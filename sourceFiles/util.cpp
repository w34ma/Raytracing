...
Matrix4x4::Matrix4x4() {//initialize as identity matrix
	for (int i = 0; i < 16; i++) 
		m_data[i] = 0.0; 
	m_data[0] = 1.0;
	m_data[5] = 1.0;
...
Colour Material::getTextureColor(Ray3D& ray) {
    return text->getColor(ray.intersection.textcoor);
}
...
Colour Texture::getColor(Point3D &coor) {
   
    unsigned int x = ((int)(coor[0] * textW) + textW) % textW;
    unsigned int y = ((int)(coor[1] * textH) + textH) % textH;
    
    
    return Colour(r[y * textW + x] / 255.0, g[y * textW + x] / 255.0, b[y * textW + x] / 255.0);
