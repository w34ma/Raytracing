...
struct Texture {
    ...
    ~Texture();
    Colour getColor(Point3D& coor);
    
    unsigned long int textW;
    long int textH;
...
struct Intersection {
    ...
    bool none;
    bool insideCrash;//to indicate it go through the object from inside or outside.if outside--false, if inside--true
    Point3D textcoor;
};
...
struct Material {
    ...
    Material( Colour ambient, Colour diffuse, Colour specular, double exp,double reflect_coef,Texture *text) :
    ambient(ambient), diffuse(diffuse), specular(specular),
    specular_exp(exp),reflect_coef(reflect_coef),transparency_coef(0),text(text) {}
    
    Material( Colour ambient, Colour diffuse, Colour specular, double exp,double reflect_coef,double transparency_coef,Texture *text) :
    ambient(ambient), diffuse(diffuse), specular(specular),
    specular_exp(exp),reflect_coef(reflect_coef),transparency_coef(transparency_coef),text(text) {}
    ...
    // Specular expoent.
    double specular_exp;
    //add coefficient of reflection, the value should between 0 and 1
    double reflect_coef;
    //transparency coeffient
    double transparency_coef;
    
    double R_index;//for refracion 
    Texture *text;
    Colour getTextureColor(Ray3D& ray);
};
